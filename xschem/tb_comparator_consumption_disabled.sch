...
N 240 100 240 120 {
lab=Vinp}
C {devices/vsource.sym} 240 150 0 0 {name=Vp 
...
C {sky130_fd_pr/corner.sym} 740 -650 0 0 {name=CORNER only_toplevel=true corner=ss}
   ...
   foreach vdd_ana_val 2.95 3.3 5.5
      ...
      foreach vm_val 0 \{$vdd_ana_val/4\} \{$vdd_ana_val/2\} \{$vdd_ana_val*3/4\} \{-0.1+$vdd_ana_val\}
         ...
         foreach ibias_val 400n
            ...
            nodeset v(x1.trim5_hv)=0 v(x1.trim4_hv)=0 v(x1.trim3_hv)=0 v(x1.trim2_hv)=0 v(x1.trim1_hv)=0 v(x1.trim0_hv)=0 v(x1.en_hv)=0 v(x1.hyst1_hv)=0 v(x1.hyst0_hv)=0
            run
            set filetype=ascii
            set appendwrite
            set wr_singlescale
...
C {devices/vsource.sym} 140 210 0 0 {name=Vm value=0}
...
C {devices/vsource.sym} -320 90 0 0 {name=Venable value=0}
