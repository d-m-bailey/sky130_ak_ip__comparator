...
*value="pulse(10m -10m 9.95u 100n 100n 50u 100u)"
...
C {devices/isource.sym} -800 -230 2 0 {name=Ibias value=200n}
...
"
   ...
   *.dc Vp -10m 10m 0.05m Vm 0 5 0.25
   *.dc Vm 0 5 0.05 Vp -100m 100m 10m
   *.dc Vp 0 5 0.01
   .tran 100n 1m
.control
   *.tran 10n 13u 9u
   ...
   *plot v(x1.vop)
   *plot v(x1.vom)
   *plot (v(x1.vop)+v(x1.vom))/2
   *plot v(x1.vop)-v(x1.vom)
   *plot v(x1.vdiff)
   meas tran current_avdd avg i(vmeas_avdd) from=500u to=1m
   meas tran current_dvdd avg i(vmeas_dvdd) from=500u to=1m
   wrdata test.txt current_avdd
.endc
.save all
"}
